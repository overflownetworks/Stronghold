...
--Position
	local hit = EffectData()
		...
		hit:SetNormal( (tr.StartPos-tr.HitPos):GetNormalized() )
		hit:SetScale(20)
...
function SWEP:Think()
	...
	self.Sprinting = self.Walking and self.Owner:KeyDown( IN_SPEED )
...
function SWEP:RecoilPower()
	...
	if self:GetIronsights() then
		...
		local punch = math.Rand(-0.1,-0.01)
		self.Owner:ViewPunch(Angle(punch, punch, punch))
	else
